...
%We calculate $p(\vec E | D,\mathcal{H},\mathcal{I})$ in two steps. First, we use an MCMC algorithm to find the posterior $p(\theta_i | \vec d_i, \mathcal{H}, \mathcal{I})$ for the waveform parameters $\theta_i$ of each binary and marginalize over any parameters that the EOS does not depend on. We next employ another MCMC that uses this marginalized likelihood to find the marginalized posterior $p(\theta_i|D,H,I)$ in the parameters $\theta = {m1,m2,log(p1),\gamma1,\gamma2,\gamma3}$.
