%\documentclass[onecolumn,prd,amssymb,aps,nofootinbib,showpacs,epsf]{revtex4}
...
[[You don't define SNR or network SNR anywhere.]]
...
Given the GW model and EOS fit described in Sections~\ref{sec:waveform} and~\ref{sec:eos}, we will describe here a method for estimating its parameters from a set of GW observations. We use Bayes' theorem to write the posterior density function (PDF) for the universal EOS parameters $\vec E = (\log(p_1), \Gamma_1, \Gamma_2, \Gamma_3)$ common to all NSs and the waveform parameters $\vec\theta = (D_L, \alpha, \delta, \psi, \iota, t_c, \phi_c, m_1, m_2, \tilde\Lambda)$ \red{[[What happened to $\delta\tilde\Lambda$? Although $\delta\tilde\Lambda$ in principle provides additional EOS information, it's effect on the waveform is small compared to $\tilde\Lambda$, so we treat it as a nuisance parameter and group it with the extrinsic parameters when we marginalize over them.]]} that are unique to each of the $n$ BNS systems as
\begin{equation}
\begin{split}
...
\end{equation}
This $10n$-dimensional integral is not easily computed, so we will now rewrite it. First we decompose the waveform parameters into extrinsic parameters $\vec\theta_{\rm ex} = (D_L, \alpha, \delta, \psi, \iota, t_c, \phi_c)$ that depend on the observer and intrinsic parameters $\vec\theta_{\rm in} = (m_1, m_2, \tilde\Lambda)$ that are relevant for measuring the EOS such that $\vec\theta = (\vec\theta_{\rm ex}, \vec\theta_{\rm in})$. 
%Although at this stage we treat the EOS parameters separately from the waveform parameters, we will later use the fact that the tidal parameter $\tilde\Lambda_i$ for system $i$ is a function $\tilde\Lambda_i = \tilde\Lambda(m_{1i}, m_{2i}, \vec E)$ of the masses of system $i$ and the EOS parameters to determine the EOS parameters. 
...
\end{equation}
The marginalized PDF then finally becomes
\begin{equation}
\label{eq:margEOS2}
\begin{split}
...
\end{equation}
If we have a GW model $m(t;\vec\theta)$ that approximates the true signal $h(t;\vec\theta)$, then the likelihood of obtaining the data $d^a(t)$ given that GW model is
\begin{equation}
p(d^a | \vec\theta, \mathcal{H}, \mathcal{I}) = p_n[d^a(t)-m(t; \vec\theta)] \propto e^{-(d^a - m(\vec\theta), d^a - m(\vec\theta))/2}.
\end{equation}
...
We evaluate Eq.~\eqref{eq:margEOS2} for the marginalized PDF in a two-step procedure similar to that described by Steiner {\it et al.}~\cite{SteinerLattimerBrown2010}, where we first evaluate the quasilikelihood (Eq.~\eqref{eq:quasilike}) for each of the $n$ BNS systems, then evaluate Eq.~\eqref{eq:margEOS2}. In the first step where we evaluate the quasilikelihood $L(\vec d_i; \vec\theta_{\rm in, i}, \mathcal{H}, \mathcal{I})$, we use the MCMC sampler \texttt{lalinference\_mcmc} included in the LSC Algorithm Library~\cite{lal}, which is specifically designed to efficiently sample the BNS parameter space\red{[[cite overview papers]]}. The priors on the extrinsic parameter are~\red{[[what are they?]]}. The priors for the intrinsic parameters $(m_{1i}, m_{2i}, \tilde\Lambda_i)$ are not contained in the quasilikelihood (Eq.~\eqref{eq:quasilike}), so we use a flat prior for the intrinsic parameters. For each binary event $i$ we perform 10 independent runs and test for convergence using the Gelman-Rubin diagnostic~\cite{GelmanRubin1992}, then join the samples. Marginalization over the extrinsic parameters is performed by generating a distribution from the samples of the three intrinsic parameters with a Gaussian kernel density estimator. In the second step, we sample the $4+2n$ parameter integrand of Eq.~\eqref{eq:margEOS2} using the affine-invarient ensemble sampler \texttt{emcee}~\cite{ForemanMackeyHogg2013}. We use the EOS priors stated in Section~\ref{sec:eos}, and uniform priors for the masses with $0.5M_\odot \le m_2 \le m_1 \le 3.0M_\odot$. We again perform 10 runs and test for convergence with the Gelman-Rubin diagnostic before joining the samples. The marginalized distribution $p(\vec E|D, \mathcal{H}, \mathcal{I})$ is then given by a histogram of the samples for $\vec E$.

The evaluation of the quasilikelihood (Eq.~\eqref{eq:quasilike}) for all $n$ events in a population is very computationally expensive. When we examine many different populations in Section V we will sometimes use the Fisher matrix approximation instead. In the large SNR limit, the difference $\Delta\vec\theta = \vec\theta - \hat\theta$ between the estimated parameters $\vec\theta$ and the true parameters $\hat\theta$ of the binary system obeys a Gaussian distribution~\cite{FinnChernoff1993}. Specifically, for $N$ parameters, the likelihood is
\begin{equation}
...
\caption{Uncertainty in the recovered EOS, radius, and tidal deformability for the three-detector aLIGO--aVirgo network. Results are shown for the loudest 20 events with Network SNR from 64.1 down to 13.7. The red, green, and blue regions represent the 68\% ($1\sigma$), 95\% ($2\sigma$), and 99.7\% ($3\sigma$) confidence regions respectively. In the bottom left panel, $p_{\rm true}$ is the pressure of the true ``injected'' EOS, which in this case is the fit to the MPA1 EOS. The tabulated MPA1 EOS is also shown. The range of masses from $1.2M_\odot$ to $1.6M_\odot$ used in the simulated BNS population is represented in the right panels by the black vertical dotted lines. The red vertical dotted line at $1.93M_\odot$ is the minimum mass, set by the prior, where some accepted EOS parameters do not produce a stable NS.}
\label{fig:mpa1fit}
\end{center}
\end{figure*}
...
\caption{Same BNS population as Fig.~\ref{fig:mpa1fit}. Contours represent 95\% confidence regions for the prior without BNS data as well as for also including the loudest 1, 5, and 20 inspiral events.}
...
\caption{Dependence of EOS confidence region on the highest observed NS mass. Results use the same 20 BNS systems as in Fig.~\ref{fig:mpa1fit}. Contours represent the 95\% confidence regions. The priors on the minimum allowed maximum mass are $M_{\rm max} \geq 1.93M_\odot$ (red) and $M_{\rm max} \geq 2.4M_\odot$ (blue). Also shown are the lower limits from just using the maximum mass and causality priors with $M_{\rm max} \geq 1.93M_\odot$ (red dashed) and $M_{\rm max} \geq 2.4M_\odot$ (blue dashed).}
\label{fig:varymmax}
\end{center}
\end{figure}
...
In addition to the above sources of systematic error which can be reduced with better modeling, there have been other suggestions that could alter the measurability of tidal parameters. Recently it was suggested, using the unipolar inductor model of Goldreich and Lynden-Bell~\cite{GoldreichLyndenBell1969}, that interactions between a NS with a large magnetic dipole and its NS companion could create a torque on the binary that could potentially lead to a phase shift in the waveform of a few cycles~\cite{Piro2012}. If true, this would likely contaminate the measurement of tidal interactions. However, as pointed out by Lai~\cite{Lai2012}, the current in the circuit between the binary pair will generate a toroidal magnetic field that breaks the circuit leading to a maximum current. Lai then found that this upper limit is orders of magnitude too small to effect the waveform of the binary.

[[Tidal excitation of g-modes~\cite{Lai1994}.]]
...
[[Cosmological expansion redshifts mass measurement, but effects tidal parameter in different way. Z can sort of be measured, but true chirp mass will be off by order 1\% (noted by Chris Messenger).]]
...
We have shown that, with a realistic population of BNS inspiral events, the advanced LIGO--Virgo GW network now undergoing construction can provide valuable information about the EOS and NS properties such as the radius and tidal deformability. Typical statistical errors in the pressure will be on the order of 10\% to a factor of 2 from 1--4 times nuclear density. This corresponds to a full width 95\% confidence of $\sim 1$--2~km for the radius and $\sim 1$--$2\times 10^{36}$~g~cm$^2$~s$^2$ for the tidal deformability over the mass range 1--$2M_\odot$. These results are in agreement with those of Ref.~\cite{DelPozzoLiAgathos2013} who found that with tens of BNS observations, $\lambda$ could be measured at a reference mass of $1.4M_\odot$ to about 10\%. However, our results show that the prior information about the EOS allows us to measure NS properties at other masses as well, and this is true even with a more realistic range of NS masses.

However, we found in Section~\ref{sec:systematic} that currently unknown high order PN point-particle terms will create a large systematic bias in recovering the EOS that will be larger than the statistical uncertainties. These terms will need to be calculated or fit with numerical simulations to accurately measure the EOS. The choice of EOS parameters can also effect the recovery of the EOS, and EOSs with complex behavior will need to be modeled with more sophisticated EOS parameterizations. Other currently unmodeled behavior such as magnetic interactions or tidally induced resonances with low frequency g-modes are unlikely to have an important effect on the waveform phase.
...
Although it is difficult to make a direct comparison with electromagnetic observations of NS masses and radii, our results indicate that advanced GW detectors can measure the NS radius with similar statistical errors. However, although both EM and GW observations have systematic errors, the gravitational waveforms from inspiralling BNS systems are an intrinsically cleaner source of data than the EM emission from NSs. Ultimately, EM and GW measurements will have to agree, and comparing the results will provide an important consistency check on our understanding of general relativity, electromagnetic radiation from NSs, and the EOS.
 
%[[In addition, the Steiner {\it et al.}~\cite{SteinerLattimerBrown2010} parameterization is very restrictive around nuclear density. Many of the theoretical EOS models that will be used in this paper cannot be easily fit to this EOS [[I think]]. By providing a very specific form for the EOS around nuclear density the Steiner {\it et al.} parameterization allows for a smaller range of pressure. However, they have used variable dividing densities at higher densities which provides a wider range of behavior.]]

\acknowledgements

[[Thank Andrew Steiner for discussion at INT.]]

BL sincerely thanks Walter Del Pozzo and Tyson Littenberg for advice and many helpful discussions as well as discussions with Chris Van Den Broeck, Tjonnie Li, and Will Farr. \red{[[Chris Messenger for discussions of redshift. The Seattle program for discussions of systematic error in general. Andrew Steiner for discussions related to the EOS parameterization.]]} The authors would also like to thank Evan Ochsner and Jolien Creighton for advice and help in simulating BNS populations. This work was supported by NSF Grants PHY-1305682, and PHY-xxxxxxx. Simulations were run on the Nemo cluster at the University of Wisconsin--Milwaukee and the Orbital cluster at Princeton University.
