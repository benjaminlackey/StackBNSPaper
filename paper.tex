...
\begin{table*}[!htb]
\begin{center}
\caption{Comparison between tabulated EOSs and their best fits. The parameters $\log(p_1/\text{(dyne~cm$^{-2}$)})$, $\Gamma_1$, $\Gamma_2$, and $\Gamma_3$ are the values that minimize the least-squares residual. Observables for the tabulated EOSs are also shown; $v_{\rm s,max}$ ($c$) is the maximum speed of sound below $\rho_{\rm c, max}$, $M_{\rm max}$ ($M_\odot$) is the maximum mass, $R_{1.4}$ (km) is the radius of a $1.4~M_\odot$ NS, and $\lambda_{1.4}$ ($10^{36}$~g~cm$^2$~s$^2$) is the tidal deformability of a $1.4~M_\odot$ NS. The percent error for each observable when using the tabulated EOS $(O_{\rm tab})$ versus the best-fit parameterized EOS $(O_{\rm fit})$ is also given by $(O_{\rm fit}/O_{\rm tab}-1)100$.}
